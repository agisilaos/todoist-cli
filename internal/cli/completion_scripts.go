...
_todoist() {
  ...
  case "$cmd" in
    ...
    filter)
      ...
      ;;
    project)
      local subs="list ls view show browse collaborators add update archive unarchive delete rm del"
      if [[ ${COMP_CWORD} -eq 2 ]]; then
        COMPREPLY=( $(compgen -W "${subs}" -- "$cur") )
        return 0
      fi
...
case $words[1] in
  ...
  filter)
    ...
    ;;
  project)
    _arguments '2:subcommand:(list ls view show browse collaborators add update archive unarchive delete rm del)' '*:flags:(--archived --id --name --description --parent --color --favorite --view --cursor --limit --all)'
    ;;
  workspace)
...
# project
complete -c todoist -n '__fish_seen_subcommand_from project; and __fish_use_subcommand' -a 'list ls view show browse collaborators add update archive unarchive delete rm del'
